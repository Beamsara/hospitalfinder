    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
          return Container();
            // Center(child: Text('ไม่พบสถานพยาบาลที่ตรงกับ "${query}"'));
        }
...
class _PharmacyPageState extends State<PharmacyPage> {
  ...
  int _currentIndex = 0; // ค่าเริ่มต้นที่หน้าหลัก
...
// ✅ ตรวจสอบว่าผู้ใช้เป็น "ผู้เยี่ยมชม"
      ...
      body:_currentIndex == 0 // ตรวจสอบว่าหน้าปัจจุบันเป็นหน้าหลักหรือไม่
          ? Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Padding(
            ...
            child: Text(
              'ร้านขายยาที่รองรับบัตรทองในจังหวัดพระนครศรีอยุธยา',
              style: TextStyle(
                fontSize: 28,
          ...
          Expanded(
            child: _currentPosition == null
              ...
              stream: FirebaseFirestore.instance
                  .collection('locations')
                  .where('type', isEqualTo: 'ร้านขายยา') // กรองเฉพาะร้านขายยา
                  .snapshots(),
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  return Center(child: CircularProgressIndicator());
                ...
                if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
                  return Center(child: Text('ไม่มีข้อมูลร้านขายยา'));
                }
