...
class GoldCardInfoPage extends StatefulWidget {
  @override
  _GoldCardInfoPageState createState() => _GoldCardInfoPageState();
}

class _GoldCardInfoPageState extends State<GoldCardInfoPage> {
  ...
  /// ✅ ดึง role ของ user และตรวจสอบสิทธิ์
  Future<void> _fetchUserRole() async {
    ...
    if (userId != null) {
      ...
      if (data != null && data.containsKey('role')) {
        ...
        // ✅ ถ้าเป็น admin ให้เปลี่ยนไปหน้า AdminHomepage()
        if (isAdmin) {
      ...
      body: Container(
        ...
        child: Center(
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              // ✅ ใช้ Shimmer Effect สำหรับ Loading Text
              Shimmer.fromColors(
              ...
              SizedBox(height: 20),
              // ✅ ใช้ CircularProgressIndicator ที่มี Glow Effect
              Container(
  ...
  Widget build(BuildContext context) {
    // ✅ ถ้ายังโหลดข้อมูล role ไม่เสร็จ → ใช้ UI Loading ใหม่
    if (userRole == 1) {
      return buildLoadingScreen(); // 🔥 ใช้ UI Loading แบบใหม่ที่สวยขึ้น
    }
    return Scaffold(
      appBar: AppBar(
        elevation: 0,
...
// ✅ ตรวจสอบว่าผู้ใช้เป็น "ผู้เยี่ยมชม"
      ...
      body: _currentIndex == 2
        ...
        builder: (context, snapshot) {
          ...
          return _buildExpandableInfoList(docs);
        },
      )
  ...
  /// ✅ **สร้าง UI ที่กดเพื่อขยาย/ซ่อนเนื้อหา**
  ...
  /// ✅ **ฟังก์ชันสร้าง UI Drag & Drop**
  Widget _buildDraggableInfoList(List<Map<String, dynamic>> items) {
    return ReorderableListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return Column(
          key: ObjectKey(item), // ✅ แก้ไข Key ให้เป็น ObjectKey เพื่อป้องกันการซ้ำ
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Padding(
              padding: EdgeInsets.symmetric(vertical: 12.0),
              child: Row(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Icon(Icons.circle, size: 10, color: Colors.teal), // จุดนำหน้าหัวข้อ
                  SizedBox(width: 10),
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(
                          ...
                          style: TextStyle(
                            fontSize: 18,
                            fontWeight: FontWeight.bold,
                            color: Colors.teal[800], // สีเขียวเข้ม
                          ),
                        ),
                        SizedBox(height: 4),
                        Text(
                  ...
                  PopupMenuButton<String>(
                    ...
                    onSelected: (value) {
                      if (value == "edit") {
                        _editInfo(item['id'], item['title'], item['content']);
                      ...
                      PopupMenuItem(
                        ...
                        child: Row(
                          children: [
                            ...
                            SizedBox(width: 8),
                            Text("แก้ไข", style: TextStyle(color: Colors.black87)),
                          ],
                        ),
                      ),
                      PopupMenuItem(
                        ...
                        child: Row(
                          children: [
                            ...
                            SizedBox(width: 8),
                            Text("ลบ", style: TextStyle(color: Colors.black87)),
                          ],
                        ),
                      ),
                    ],
            ...
            Divider(height: 1, thickness: 1, color: Colors.teal[100]), // เส้นคั่นสีเขียวมิ้นท์บาง ๆ
          ],
        );
      },
    );
