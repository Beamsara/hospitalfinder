  ...
  Widget buildResults(BuildContext context) {
    ...
    return StreamBuilder<QuerySnapshot>(
      stream: FirebaseFirestore.instance
          ...
          .where('name', isGreaterThanOrEqualTo: query) // กรองชื่อเริ่มต้น
      ...
      builder: (context, snapshot) {
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final guest = null; // กำหนด guest เป็น 'null'
            debugPrint('🔥 Current User ID: $currentUserId');

            // ตรวจสอบว่า type เป็น 'คลินิก'
            if (data['type'] == 'คลินิก') {
              return Card(
                ...
                child: ListTile(
                  ...
                  trailing: Row(
                    ...
                    children: [
                      // ✅ เช็คว่า userId ตรงกับ currentUserId หรือไม่
                      ...
                      // ✅ เช็คว่า user เป็น guest หรือไม่
                      if (data['userId'] == null)
                        Container(
                          margin: EdgeInsets.only(right: 8),
                          padding: EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                  ...
                  onTap: () {
                    _showPinDetailsDialog(context, documents[index]);
                    debugPrint('🔥 Current User ID: $currentUserId');
                  },
                ),
              );
            ...
            // ถ้า type ไม่ใช่ 'คลินิก' ให้คืนค่าเป็น Container ว่าง
            return Container();
          },
        );
...
class _ClinicPageState extends State<ClinicPage> {
  ...
  int _currentIndex = 1; // ค่าเริ่มต้นที่หน้าหลัก
...
// ✅ ตรวจสอบว่าผู้ใช้เป็น "ผู้เยี่ยมชม"
      ...
      body:_currentIndex == 1 // ตรวจสอบว่าหน้าปัจจุบันเป็นหน้าหลักหรือไม่
          ? Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Padding(
            ...
            child: Text(
              'คลินิกที่รองรับบัตรทองในจังหวัดพระนครศรีอยุธยา',
              style: TextStyle(
                fontSize: 28,
          ...
          Expanded(
            child: _currentPosition == null
              ...
              stream: FirebaseFirestore.instance
                  .collection('locations')
                  .where('type', isEqualTo: 'คลินิก') // กรองเฉพาะคลินิก
                  .snapshots(),
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  return Center(child: CircularProgressIndicator());
                ...
                if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
                  return Center(child: Text('ไม่มีข้อมูลคลินิก'));
                }
                ...
                // คำนวณระยะทางและเรียงลำดับ
                final sortedDocuments = documents.map((doc) {
                  final data = doc.data() as Map<String, dynamic>; // แปลงข้อมูล
                  final distance = _calculateDistance(
                    data['latitude'],
                    data['longitude'],
                  );
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    // ✅ แสดงค่า currentUserId ออกทาง Console
                    debugPrint('🔥 Current User ID: $currentUserId');
                    debugPrint('🔥 Data User ID: ${data['userId']}');
                      ...
                      child: ListTile(
                        ...
                        trailing: Row(
                          ...
                          children: [
                            ...
                            // ✅ เช็คว่า user ไม่ได้เข้าสู่ระบบ (currentUserId == null)
                            if (data['userId'] == null)
                              Container(
                                margin: EdgeInsets.only(right: 8),
                                padding: EdgeInsets.symmetric(horizontal: 8, vertical: 4),
